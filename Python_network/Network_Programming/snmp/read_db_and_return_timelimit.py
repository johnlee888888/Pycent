...
def read_db(db, name, passwd, ho, set_time=1):
    ...
    # print(time_range)
    cursor.execute("select time,cpu from cpu_total where time > '{0}'".format(time_range))
...
if __name__ == '__main__':
    data_1 = read_db(db='pycent', name='admin', passwd='Pa$$w0rd', ho='localhost')  # 得到数据库中的值
    r1 = [(datetime.datetime.strptime(a, "%Y-%m-%d %H:%M:%S.%f"), b) for a, b in data_1]  # 将时间格式转换
    plot_data(r1)

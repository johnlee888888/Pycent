...
def login_dev(ip, username, password, port, cmd):
    try:
        ...
        if re.match(r'\s*Line\s+\w{1,3}\s+invalid\s+\w+\s+', config):
            return '命令不能被执行! 请检查命令或者用户权限!\nNone'
        elif re.findall(r'\s+Ambiguous\s+\w{1,7}', config):
...
if __name__ == '__main__':
    print(login_dev('192.168.107.131', username='admin', password='admin', port=22, cmd='show v'))
